...
class Node extends Memcached {
	...
	public function allKnownNodes($page, $orderby)
	{
		...
		if($paginate->success == true) {
			...
			if( $result > 0) {
				$resp['data'] = $result; 
				$resp['pagination'] = $paginate->links_html;
				$this->m->set("h-nodes-all-pg{$page}-ob{$orderby}", $resp, time() + 400);
				return $resp;
			}
			else {
				return false;
	...
	public function get($ip)
	{
		...
		$stmt = $db->prepare("SELECT addr, hostname, ownername, first_seen, last_seen, country, public_key, version, latency, lat, lng, map_privacy, profile_header_img  from nodes where addr = :ip");
	...
	public function getLatencyGraph($ip) {
		...
		$stmt = $this->db->prepare("(SELECT ts, latency FROM pings WHERE ip = :ip ORDER BY ts DESC LIMIT 20) order by ts");    
	...
	public function getVersionGraph($ip) {
		...
		$dbh = $this->db;
		$stmt = $dbh->prepare('SELECT protocol, ts FROM pings WHERE ip = :ip  GROUP BY protocol ORDER BY protocol ASC LIMIT 10');    
		$stmt->bindParam(':ip', $ip);   
		if(!$stmt->execute()) {
	...
	public function getNodeList($limit = 50) {
	...
	public function getDiscoveryList($limit = 50) {
		$limit = (is_int($limit)) ? $limit : 50;
		$stmt = $this->db->prepare(
			'SELECT addr 
			FROM nodes 
			WHERE addr NOT IN (SELECT ip FROM pings WHERE ip NOT IN ("fcec:ae97:8902:d810:6c92:ec67:efb2:3ec5"))
			ORDER BY RAND()
			LIMIT '.$limit.';');
		$stmt->execute();
		$list = $stmt->fetchAll(PDO::FETCH_COLUMN);
	...
	public function getPingList($limit = 50) {
		$db = $this->db;
		$limit = (is_int($limit)) ? $limit : 50;
		$stmt = $db->prepare(
			'SELECT DISTINCT(ip) 
			FROM pings 
	        		WHERE ip NOT IN ("fcec:ae97:8902:d810:6c92:ec67:efb2:3ec5")
			ORDER BY RAND()
			LIMIT '.$limit.';');
		$stmt->execute();
		$list = $stmt->fetchAll(PDO::FETCH_COLUMN);
	...
	public function pingNode($ip,$rip, $timeout = 100)
	{
		$timeout = (is_int($timeout)) ? $timeout : 100;
		$rip = 'fcec:ae97:8902:d810:6c92:ec67:efb2:3ec5';
		$capi = new CjdnsApi();
		$ping = $capi->call("RouterModule_pingNode", [
			"path"=>$ip,
			"timeout" => $timeout
			]);
		if(isset($ping['result']) && $ping['result'] === "pong") {
			$timestamp = date('c');
			...
			$dbh = $this->db->prepare('insert into pings (ts, ip, nodepath, latency, protocol, request_ip) 
			...
			$dbh->bindParam(":ts", $timestamp, PDO::PARAM_STR);
			$dbh->bindParam(":addr", $ip, PDO::PARAM_STR);
			$dbh->bindParam(":path", $path, PDO::PARAM_STR);
			$dbh->bindParam(":latency", $latency, PDO::PARAM_INT);
			$dbh->bindParam(":protocol", $protocol_version, PDO::PARAM_INT);
			$dbh->bindParam(":rip", $_SERVER['REMOTE_ADDR'], PDO::PARAM_STR);
			$dbh->bindParam(':public_key', $public_key , PDO::PARAM_STR);
			if(!$dbh->execute())
			{
				return false; 
			}
	...
	{
		$db = $this->db->prepare(
			'SELECT DISTINCT(peers.peer_pubkey), nodes.addr, nodes.version, nodes.hostname
			FROM peers 
			LEFT JOIN nodes 
